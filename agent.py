...
class Agent:
    ...
    def select_action(self):
        ...
        action = np.clip(noisy_action, self.actions_boundaries[0], self.actions_boundaries[1])
        return action

    def sample_noise(self):
        return np.random.normal(loc=0., scale=self.noise_scale, size=(self.n_agents, self.world.action_space_size))
    ...
    def train(self):
        ...
        for i in range(1, self.nr_episodes + 1):
            ...
            if i % self.output_freq == 0:
                ...
                avg_scores = np.mean(np_scores, axis=1)
                score_to_print = round(np.mean(avg_scores), 2)
    ...
    def run(self):
        ...
        while not self.finished:
            ...
            if done:
                self.finished = True
                print(f'Reacher got on average {np.mean(total_reward)} points!')
